...
sudo mkdir -p '$APP_ROOT/releases' '$APP_ROOT/shared/env' '$APP_ROOT/shared/logs/web' '$APP_ROOT/shared/logs/worker' '$APP_ROOT/shared/run'
...
if [ ! -f '$APP_ROOT/shared/env/web.env' ]; then
  ...
  echo 'Created empty $APP_ROOT/shared/env/web.env. Fill it before the first successful build.' >&2
fi
