...
| `GET/POST/PATCH /api/connectors` | MCP-конектори і доступ ролей (`?role=` для worker routing) |
...
- MCP-конектори: `connectors` + `connector_role_access`;
- UI-фічі: `role_features` (`training`, `connectors_manage`, `export`, …).
...
src/
  app/
    ...
    api/                   — chat, tasks/*, runs, mail, agents, connectors, features
...
| 025 | ренейм runtime-сутності KB → connectors: `connectors`, `connector_role_access`, `/api/connectors`, `connectors_manage` |
...
| Роль | `connector_role_access` / `role_agent_access` vs `profiles.role` до виклику |
