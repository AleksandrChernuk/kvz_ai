...
# Незворотна дія в тексті кроку? (запис у 1С/Bitrix, ціна клієнту, оплата,
# .dxf на верстат/лазер). Якщо так — задача має пройти гейт людини ДО виконання.
is_irreversible() { # $1 = текст
  # Реагуємо на ДІЮ (відправити/записати/виконати назовні), а не на просту
  # згадку слова. Інакше порада «розкажи про лазерне різання» хибно блокувалась.
  case "$(printf '%s' "$1" | tr '[:upper:]' '[:lower:]')" in
    # оплата/гроші
    *оплат*|*платіж*|*проплат*|*сплат*|*перекаж*кошт*)
      return 0 ;;
    # ціна/рахунок/КП клієнту (вихід назовні)
    *ціну*клієнт*|*ціну*замовник*|*рахунок*клієнт*|*рахунок*замовник*|*комерційн*пропозиц*|*інвойс*)
      return 0 ;;
    # запис у облікові/CRM системи
    *запиш*1с*|*запис*1с*|*у*1с*|*в*1с*|*до*1с*|*у*bitrix*|*в*bitrix*|*до*bitrix*|*у*бітрікс*|*в*бітрікс*|*створ*угод*|*створ*лід*)
      return 0 ;;
    # відправка програми/файлу на верстат/ЧПУ/лазер (реальна дія)
    *на*верстат*|*на*чпу*|*на*лазер*|*на*плазм*|*відправ*верстат*|*відправ*лазер*|*відправ*чпу*|*передай*верстат*|*.dxf*на*|*вигрузи*верстат*)
      return 0 ;;
    *) return 1 ;;
  esac
}
...
# Незворотний крок (ціна клієнту, .dxf на верстат, запис у 1С/Bitrix, оплата) та
